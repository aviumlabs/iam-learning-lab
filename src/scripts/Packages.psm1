...
# Additional the Packages module installs and configures the 
...
$ADDomain = [ordered]@{
    ...
    "GroupOU" = "Groups";
    "ServiceLogonGroup" = "LogonAsService";
...
# the secrets directory .secret_manager_pass, .secret_rpa_pass
$TomcatUsers = [ordered]@{
    "Manager" = "manager";
    "ManagerPass" = "";
    "RpaUser" = "rpa-tomcat";
...
$BasePackages = [ordered]@{
    ...
    "VSCodeSetup-x64-1.98.2.exe" = @{ 
        endpoint = "https://update.code.visualstudio.com/1.98.2/win32-x64/stable";
        halg = "SHA256"
        vhash = "2cfa385732dac31e82d94808af04f129f897aadd74b1967fb58035c8c615555e";
        verified = $false;
    }
}
...
"@
    ...
    # Creates the default svc-tomcat service account
    New-ADServiceAccount -Path $Path
    # Creates the default LogonAsService security group
    New-ADSecurityGroup
    Add-MemberToSecurityGroup
    # Applies the Development Server Group Policy
    Initialize-DevServerGPO
    # Applies the Logon As A Service Group Policy
    # Required for running Apache Tomcat under specified service account
    Initialize-LogonServiceGPO
...
function Add-MemberToSecurityGroup {
    ...
    if (-Not $Member) {
        $tc_svc_acct = $ADDomain["TomcatUser"]
        $Member = Get-ADUser $tc_svc_acct
    }
    if (-Not $SecurityGroup) {
        $logon_group = $ADDomain["ServiceLogonGroup"]
        $SecurityGroup = Get-ADGroup $logon_group
    }

    Write-Host "Adding member $Member.Name to $SecurityGroup.Name ..."
    Add-ADGroupMember -Identity $SecurityGroup -Members $Member | Out-Null

    if ($?) {
        Write-Host "Member $Member.Name has been added to $SecurityGroup.Name ."
    } else {
        Write-Host "Failed to add member $Member.Name to $SecurityGroup.Name ."
    }   
}
...
function Initialize-ApacheTomcat {
    ...
    --StdOutput "$server_name-identityiq-stdout.YEAR-MONTH-DAY.log" `
    --StdError "$server_name-identityiq-stderr.YEAR-MONTH-DAY.log" | Out-Null
    Write-Host "Apache Tomcat Windows service update completed."

    # Start Tomcat Service to create Engine directory
    tomcat9 //ES//IdentityIQ
    tomcat9 //SS/IdentityIQ
    ...
    $manager_xml_path = "$env:CATALINA_BASE\conf\Catalina\$server_name\manager.xml"
    $manager_xml_content = @"
...
function Initialize-ApacheManagerHTML {
    ...
    # this regex is yet working
    $match_content = "(<!-- Uncomment this to set a sub-title[\s\S]*?valid HTML.)"
    ((Get-Content -Path $manager_web_xml_path -Raw) -Replace $match_content, ('$1' + "`n    -->`n")) `
    | Set-Content -Path $manager_web_xml_path
}
...
<#
.SYNOPSIS 
    Internal function to initialize computer settings.
.DESCRIPTION
    Internal function to initialize computer settings. Removes the requirement 
    to press CTRL-ALT-DEL to login and lock screen timeout - NOT suitable for 
    production. 
.PARAMETER Path
    Root path of the lab installation.
.PARAMETER DomainDn
    The DN of the Active Directory domain to link the GPO. Defaults to 
    the RootDN defined in the ADDomain dictionary at the top of this 
    script.
.EXAMPLE
    Initialize-DevServerGPO
    Initialize-DevServerGPO -DomainDn "DC=aviumlabs,DC=com"
#>
function Initialize-DevServerGPO {
    param (
        [string]$DomainDn
    )
    if (-Not $DomainDn) {
        $DomainDn = $ADDomain["RootDn"]
    }
    Write-Host "Initializing Development Server group policy..."
    # Export the GPO, documentation only
    # $path = "$PSScriptRoot\GPO\DevelopmentServer”
    # Backup-Gpo -Name "Development Server Policy” -Path " $path -Comment "Development Server Policy”
    ...
    -Path $gpo_path -CreateIfNeeded | Out-Null
    ...
    if ($?) {
        ...
        # environment
        Get-GPO $policy_name | New-GPLink -Target $DomainDn -LinkEnabled Yes | Out-Null
...
<#
.SYNOPSIS 
    Internal function to initialize logon as a service group policy.
.DESCRIPTION
    Internal function to initialize logon as a service group policy. Dependent 
...
.EXAMPLE
    Initialize-LogonServiceGPO
    Initialize-LogonServiceGPO -DomainDn "DC=aviumlabs,DC=com"
#>
function Initialize-LogonServiceGPO {
    param (
        [string]$DomainDn
    )
    if (-Not $DomainDn) {
    ...
    Write-Host "Initializing Logon As A Service group policy..."
...
.EXAMPLE
    New-ADSecurityGroup 
    New-ADSecurityGroup -GroupName "LogonAsService" -Description "Logon as a service security group" -DisplayName "Logon As A Service Security Group"
#>
function New-ADSecurityGroup {
    ...
    if (-Not $GroupName) {
        $GroupName = $ADDomain["ServiceLogonGroup"]
    }
    if (-Not $Description) {
        $Description = "Logon as a service security group."
    }
    if (-Not $DisplayName) {
        $DisplayName = $ADDomain["ServiceLogonGroup"]
    }
...
.DESCRIPTION
    Internal function to create a new AD service account, by default it 
    creates the Apache Tomcat service account, generates a random password, 
    and saves the random password to the secrets directory - 
    .secret_tomcat_svc_pass. 
.PARAMETER Path
    The root installation path. 
.PARAMETER AccountName
    SamAccountName of the service account to create.
.PARAMETER Description
    The purpose of the service account. 
.EXAMPLE
    New-ADServiceAccount -Path "C:\"
    New-ADServiceAccount -Path "C:\" -AccountName "svc-httpd" -Description "Apache HTTPD service account$?"
#>
function New-ADServiceAccount {
    ...
    $s_pass = ConvertTo-SecureString "$svc_pass" -AsPlainText -Force
    $sa_ou = "OU=" + $ADDomain["ServiceOU"] + "," + $ADDomain["RootDN"]

    $SvcParams = @{
        AccountPassword = $s_pass
...
function Set-PermanentEnvVariables {
    ...
    $psql_bin_path = $bin_path + "\postgresql\15"
